...
let answerString = "";
const returnAnswer = () => {
  //   const randomNumber = Math.trunc(Math.random() * capitalsWordsArr.length);
  answerString =
...
const applyGreyToWrongLetterInputKeys = (input) => {
  ...
  const correctCharclass = "input-grid__button--correct";
...
const applyColorForAnswerCheckFeedback = (correctness, indexCount) => {
  ...
  if (correctness == "match") {
    boxToColor.style.backgroundColor = "#538d4e";
  } else if (correctness == "almost") {
    boxToColor.style.backgroundColor = "#b59f3b";
    ...
    boxToColor.style.backgroundColor = "#3a3a3c";
...
const checkIfLastGuessOfGame = (inputArray) => {
  ...
  if (isLastGuess) {
    alert("Sorry game over. Hit restart to try again.");
    scoreBoard.innerHTML = `<p>SCORE: 0 </p>`;
  } else {
